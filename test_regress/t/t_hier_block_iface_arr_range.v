...
// Range-form interface ARRAY boundary port ([hi:lo], parses to UnpackArrayDType,
// unlike size-form [N] which parses to BracketArrayDType). Flattened per element.
...
module sub_arr (bus_ifs.sink in[1:0], bus_ifs.source out[1:0]);
...
module t;
  ...
  bus_ifs in_ifs[1:0] (.clk(clk));
  bus_ifs out_ifs[1:0] (.clk(clk));
